...
def get_training_posest(basedir, translation=0.0, sc_factor=1.0, trainskip=1):
    all_poses, valid = load_poses(os.path.join(basedir, 'neural_pose.txt'))
...
def get_num_training_framest(basedir, trainskip):
    poses = get_training_posest(basedir, trainskip=trainskip)

    return poses.shape[0]


def get_intrinsics(basedir, crop):
    depth = imageio.imread(os.path.join(basedir, 'depth', '1305031453.374112.png'))
    H, W = depth.shape[:2]
    H = H - crop / 2
    W = W - crop / 2
